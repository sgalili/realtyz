DROP FUNCTION IF EXISTS public.get_affiliate_marketplace_v3();
DROP FUNCTION IF EXISTS public.get_affiliate_marketplace_map(uuid);
DROP FUNCTION IF EXISTS public.get_affiliate_referral_listings();

CREATE OR REPLACE FUNCTION public.get_affiliate_marketplace_v3()
 RETURNS TABLE(listing_id uuid, broker_id uuid, property_title text, address text, city text, neighborhood text, property_type text, deal_type text, rooms numeric, sqm numeric, asking_price numeric, image_url text, media_photos jsonb, slug text, reward_type text, reward_amount numeric, approved_at timestamp with time zone, tier1_amount numeric, tier2_amount numeric, tier3_type text, tier3_amount numeric, tier4_type text, tier4_amount numeric, broker_name text, office_name text, broker_license_number text, agency_logo_url text, latitude numeric, longitude numeric)
...
AS $function$
  SELECT
    ...
    l.affiliate_tier4_type,
    l.affiliate_tier4_amount,
...
CREATE OR REPLACE FUNCTION public.get_affiliate_marketplace_map(_user_id uuid)
 RETURNS TABLE(listing_id uuid, broker_id uuid, property_title text, address text, city text, neighborhood text, property_type text, deal_type text, rooms numeric, sqm numeric, asking_price numeric, image_url text, media_photos jsonb, slug text, reward_type text, reward_amount numeric, approved_at timestamp with time zone, tier1_amount numeric, tier2_amount numeric, tier3_type text, tier3_amount numeric, tier4_type text, tier4_amount numeric, broker_name text, office_name text, broker_license_number text, agency_logo_url text, latitude numeric, longitude numeric)
 LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $function$
  SELECT
  ...
  WHERE l.affiliate_enabled = true
    AND COALESCE(l.status, 'live') = 'live'
    AND public.is_affiliate(_user_id)
  ORDER BY l.affiliate_approved_at DESC NULLS LAST, l.created_at DESC
$function$;
...
GRANT EXECUTE ON FUNCTION public.get_affiliate_marketplace_v3() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_affiliate_marketplace_map(uuid) TO authenticated, service_role;
