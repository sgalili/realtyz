-- 1. Facebook Page binding: workspace -> own account -> platform shared
...
BEGIN
  ...
  -- Account-level sharing: the signed-in user's own binding works in every workspace
  RETURN QUERY
  SELECT b.page_id::text, b.page_name::text, b.page_avatar_url::text,
         length(COALESCE(b.page_access_token, '')) > 30, false, b.owner_id
    FROM public.messenger_page_bindings b
   WHERE b.owner_id = _uid
   ORDER BY b.updated_at DESC
   LIMIT 1;
...
-- 2. Account-level integration status shared across every workspace of the user
...
BEGIN
  ...
  SELECT jsonb_build_object(
           ...
           'is_shared', COALESCE(b.is_platform_shared, false))
    INTO _fb
    FROM public.messenger_page_bindings b
   WHERE b.owner_id = ANY(_owners) OR b.is_platform_shared = true
   ORDER BY (b.owner_id = _uid) DESC, COALESCE(b.is_platform_shared, false), b.updated_at DESC
   LIMIT 1;
