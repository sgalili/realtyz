...
CREATE OR REPLACE FUNCTION public.crm_safe_int(_t text)
RETURNS integer
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT round(public.crm_safe_numeric(_t))::integer
$$;
  ...
  FOR p IN SELECT * FROM jsonb_array_elements(data->'properties') LOOP
    ...
    IF v_listing IS NULL THEN
      ...
      VALUES (v_owner, v_slug,
        ...
        coalesce(public.crm_safe_numeric(p->>'price'), public.crm_safe_numeric(p->>'asking_price'), 0),
        p->>'city', nullif(p->>'neighborhood',''), nullif(p->>'address',''),
        public.crm_safe_numeric(p->>'rooms'), public.crm_safe_int(p->>'built_area_sqm'), public.crm_safe_int(p->>'floor'),
    ...
    ELSE
      UPDATE public.listings SET
        ...
        asking_price = coalesce(public.crm_safe_numeric(p->>'price'), public.crm_safe_numeric(p->>'asking_price'), asking_price),
        ...
        rooms = coalesce(public.crm_safe_numeric(p->>'rooms'), rooms),
        sqm = coalesce(public.crm_safe_int(p->>'built_area_sqm'), sqm),
        floor = coalesce(public.crm_safe_int(p->>'floor'), floor),
