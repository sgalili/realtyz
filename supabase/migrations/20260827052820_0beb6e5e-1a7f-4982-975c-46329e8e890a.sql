...
GRANT SELECT, INSERT ON public.crm_import_staging TO authenticated;
GRANT ALL ON public.crm_import_staging TO service_role;
ALTER TABLE public.crm_import_staging ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "crm_import_staging_admin" ON public.crm_import_staging;
CREATE POLICY "crm_import_staging_admin" ON public.crm_import_staging
...
BEGIN
  ...
  FOR p IN SELECT * FROM jsonb_array_elements(data->'properties') LOOP
    ...
    IF v_listing IS NULL THEN
      ...
      INSERT INTO public.listings (user_id, slug, is_published, property_title, description,
        ...
        deal_type, status, source, external_id, office_notes, additional_details, owner_id, available_from)
      VALUES (v_owner, v_slug,
        ...
        jsonb_build_object('crm_import', p), v_owner,
  ...
  -- ============ INTERACTIONS -> interaction_activity_log ============
...
END;
$fn$;
GRANT EXECUTE ON FUNCTION public.crm_import_merge() TO service_role;