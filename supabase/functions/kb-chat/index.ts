...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = [
      "אתה עוזר AI שעונה בעברית, אך ורק על סמך מאגר הידע (KB) של המשתמש המצורף בהקשר.",
      "אסור להמציא מידע. אם התשובה לא נמצאת בהקשר, ענה: 'לא מצאתי מידע על כך במאגר הידע.'",
      "צטט בקצרה את שם המסמך הרלוונטי בסוגריים מרובעים בסוף משפטים מבוססי-עובדה.",
      "אסור: מקפים אם-דאש/אן-דאש, ביטויי AI גנריים, או מענה ממידע חיצוני.",
      "",
      context ? `הקשר ממאגר הידע:\n${context}` : "אין הקשר רלוונטי במאגר הידע.",
    ].join("\n");
