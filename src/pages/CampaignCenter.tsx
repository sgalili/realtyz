  ...
  const handleGenerateFirstComment = async (postBody?: string) => {
    ...
    try {
      ...
      const styleInstructions = [
        'כתוב "תגובה ראשונה" (First Comment) קצרה מאוד לפוסט נדל"ן. המטרה היחידה שלה: SEO — לשתול באופן טבעי מילות מפתח לחיפוש (סוג הנכס, עיר, שכונה, מספר חדרים, "מתווך"/"תיווך" באזור), מבלי שזה יראה כמו רשימת מילות מפתח.',
        'אורך: 2–4 שורות קצרות בלבד. שורה אחת של המלצה אישית חמה בגוף ראשון של אודי ויטמן (למשל "נכס שכיף לי להציע", "מיקום שאני מאוד אוהב"), ואז משפט טבעי שמשלב את מילות המפתח.',
        'איסור מוחלט: אל תחזור על גוף הפוסט. אל תעתיק שורות מהפוסט. בלי בולטים, בלי ✅, בלי רשימות תכונות. אל תחזור על המחיר, המ״ר, הכתובת המלאה או תיאור הדירה — הם כבר בפוסט.',
        'סיים תמיד בשורת חתימה: "אודי ויטמן | 052-2973500".',
        'אל תשתמש בכוכביות, במקפים כפולים (--) או ב-em-dash. עברית תקינה בלבד.',
        'דוגמת מבנה (השראה בלבד, אל תעתיק): "אחד הנכסים שהכי כיף לי להציע לאחרונה. אם אתם מחפשים דירת 4 חדרים בהרצליה, בשכונת גליל ים — זה בדיוק סוג הנכס שכדאי להכיר. אודי ויטמן | 052-2973500"',
