...
// LAW #1 (frontend mirror): strip building/house/apartment numbers from any
// street address before it ships to a prompt, keyword line, or preview UI.
// Keeps the street name, drops every trailing numeric token
// ("אריה לייב יפה 36 2" → "אריה לייב יפה"). Never modifies units like
// "4 חדרים", "120 מ\"ר", "קומה 3", prices, or percentages.
const stripAddressNumbers = (value: unknown): string => {
  let s = normalizeListingText(value);
  if (!s) return s;
