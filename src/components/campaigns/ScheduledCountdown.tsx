...
/**
 * Strict dd/hh/mm countdown to a scheduled broadcast. No labels, no wording —
 * just the remaining days/hours/minutes.
 */
export function ScheduledCountdown({ iso, className }: { iso: string; className?: string }) {
  ...
  const countdown = `${pad(Math.floor(diff / 86_400_000))}/${pad(Math.floor((diff % 86_400_000) / 3_600_000))}/${pad(Math.floor((diff % 3_600_000) / 60_000))}`;
