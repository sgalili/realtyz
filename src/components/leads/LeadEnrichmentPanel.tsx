...
export default function LeadEnrichmentPanel({ lead, hideEnrichmentButton }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Contact & Location — phone sits in the same row, right before email */}
      <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                <PhoneIcon className="h-3.5 w-3.5 text-slate-700" /> טלפון
                {savingField === 'phone' && <Loader2 className="h-3 w-3 animate-spin text-muted-foreground" />}
              </Label>
              <Input
                type="tel"
                inputMode="tel"
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center justify-center h-8 w-8 rounded-md bg-transparent text-indigo-600 hover:bg-indigo-50 transition-colors"
      >
        <Sparkles className="h-4 w-4" strokeWidth={1.8} />
      </button>
      <LeadEnrichmentDialog open={open} onOpenChange={setOpen} lead={lead} />
    </>
  );
}
