        ...
        map.on('load', () => {
          ...
          map.on('mouseenter', 'leads-circles', (e: any) => {
            ...
            popup.setLngLat(e.features[0].geometry.coordinates).setHTML(
              `<div style="direction:rtl;font-family:sans-serif;padding:4px"><strong>${p.city}</strong><br/>אנשי קשר: ${p.count}<br/>חיובי: ${p.positive} | ניטרלי: ${p.neutral} | שלילי: ${p.negative}</div>`
            ).addTo(map);
          });
